...
Laser Map Maker emits each contour three times (stroked outline + inverse-fill mask hole + positive fill), and uses a single stroke color, so xTool Studio can't tell what to cut vs. engrave. This app:

1. Deduplicates the geometry (drops the canvas-mask rectangle and reversed-winding duplicates).
2. Splits the remaining contours into **cut** (red, `#FF0000`) and **engrave** (black, `#000000`) by geometric nesting — a contour that contains another is the engrave guide for the next-higher layer; everything else is cut.
